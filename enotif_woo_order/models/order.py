...
class Order(models.AbstractModel):
    ...
    def add_order(self, data):
      ...
      if data.get('line_items', []):
        params = {'include' : data.get('line_items', [])[0].get('product_id',0), 'per_page':100}
      else:
        params = {}
      headers = {'User-Agent': 'odoo_enotif_request'}
      keys = (woocommerce_api_key, woocommerce_api_secret)
      product_details = requests.get(url, params=params, auth=keys, headers=headers, timeout=URLOPEN_TIMEOUT, verify=False)
      if product_details.json() and product_details.json()[0].get('categories'):
        categ_name = product_details.json()[0].get('categories')[0].get('name')
      company_id = self.env["res.company"].sudo().search([('name', '=', categ_name)])
      if not company_id:
